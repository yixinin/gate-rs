...
pub struct ClientState {
    ...
    pub token_userid: HashMap<String, i64>,
}

impl Default for ClientState {
    fn default() -> ClientState {
        ClientState {
            connid_socket: HashMap::new(),
            userid_connid: HashMap::new(),
            connid_userid: HashMap::new(),
            token_userid: HashMap::new(),
        }
    }
}
...
impl ClientState {
    ...
    pub fn add_client(&mut self, userid: i64, sender: Sender) -> Result<(), (Sender, &str)> {
    ...
    pub fn disconnect(&mut self, userid: i64) {
        if let Some(sender) = self.get_sender_by_userid(userid) {
            sender.close(ClientState::GAME_CLOSE_CODE).ok();
        }
